...
function startThumbnailCacheRefresh() {
  ...
  cacheRefreshPromise = (async () => {
    try {
      ...
      console.error("[tradesThumbnailCache] Error:", error?.message || error);
    } finally {
  ...
  await Promise.all(
    marketIds.map(async (id) => {
      try {
        ...
        console.error("[tradesThumbnailCache] onDemand fetch failed:", error?.message || error);
      }
    }),
  );
...
export async function GET(request, { params }) {
  try {
    ...
    const data = await opinionFetch(`/trade/user/${wallet}`, {
