...
if (!hub) {
  hub = {
    ...
    // ✅ NEW: Track hourly markets separately (always subscribed when active)
    subscribedHourlyIds: new Set(),      // hourly binary marketIds
...
// ✅ Hourly markets refresh more frequently (every 10 minutes)
const HOURLY_REFRESH_MS = 10 * 60 * 1000;
...
/**
 * Check if a market is an hourly market based on title
 * Hourly markets typically have "Hourly" in title and expire within 2 hours
 */
function isHourlyMarket(m) {
  const title = getMarketTitle(m) || "";
  const titleLower = title.toLowerCase();
  
  // Check if title contains "hourly" 
  if (!titleLower.includes("hourly")) return false;
  
  // Optionally check cutoff time is within 2 hours
  const cutoff = m?.cutoffAt ?? m?.cutoff_at ?? 0;
  if (cutoff) {
    const cutoffMs = cutoff < 1e12 ? cutoff * 1000 : cutoff;
    const now = Date.now();
    const twoHoursMs = 2 * 60 * 60 * 1000;
    // If cutoff is in the past or more than 2 hours away, might not be current hourly
    if (cutoffMs < now || cutoffMs > now + twoHoursMs) {
      return false;
    }
  }
...
/**
 * Fetch all active hourly markets
 * These are short-lived markets (1 hour) that should always be tracked
 */
async function fetchActiveHourlyMarkets() {
  const hourlyMarkets = [];
  const seen = new Set();
  
  console.log(`[SmartMoney] Fetching active hourly markets...`);
  
  // Fetch activated binary markets and filter for hourly
  for (let page = 1; page <= 5; page++) { // Only check first few pages
    ...
    for (const m of list) {
      ...
      if (isHourlyMarket(m)) {
        ...
        hourlyMarkets.push({ id, title });
      }
    }
  }
  
  console.log(`[SmartMoney] Found ${hourlyMarkets.length} active hourly markets`);
...
/**
 * Sync hourly market subscriptions
 * - Subscribe new hourly markets
 * - Unsubscribe expired hourly markets
 */
async function syncHourlySubscriptions() {
  if (!hub.ws || hub.ws.readyState !== WebSocket.OPEN) return;

  const hourlyMarkets = await fetchActiveHourlyMarkets();
  const nextHourlySet = new Set(hourlyMarkets.map(m => m.id));

  // Unsubscribe expired hourly markets
  let removed = 0;
  for (const oldId of hub.subscribedHourlyIds) {
    if (!nextHourlySet.has(oldId)) {
      // Check if it's also in regular binary subscriptions - don't unsub if so
      if (hub.subscribedBinaryIds.has(oldId)) continue;
      
      hub.subscribedHourlyIds.delete(oldId);
      removed++;
      try {
        hub.ws.send(
          JSON.stringify({
  ...
  // Subscribe new hourly markets
  let added = 0;
  for (const id of nextHourlySet) {
    // Skip if already subscribed via regular binary or hourly
    if (hub.subscribedBinaryIds.has(id) || hub.subscribedHourlyIds.has(id)) continue;
    
    hub.subscribedHourlyIds.add(id);
    added++;
    try {
      hub.ws.send(
        JSON.stringify({
  ...
  if (added > 0 || removed > 0) {
    console.log(
      `[SmartMoney] Hourly sync: ${hub.subscribedHourlyIds.size} hourly markets (+${added}, -${removed})`
    );
  }
}
...
async function refreshTopSubscriptions(force = false) {
  ...
  // Also sync hourly markets
  await syncHourlySubscriptions();
}
...
export function getSmartMoneyHubStatus() {
  ...
  const hourlyIds = Array.from(hub.subscribedHourlyIds || []);
  ...
  return {
    ...
    subscribedHourlyCount: hourlyIds.length,
    subscribedTotalCount: binaryIds.length + rootIds.length + hourlyIds.length,
    ...
    // ✅ Sample hourly markets
    hourlyMarketsSample: hourlyIds
      .slice(0, 10)
      .map((id) => ({
        id,
        title: hub.marketTitleById.get(id) || "",
