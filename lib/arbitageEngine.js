...
async function getOpinionBestBid(tokenId) {
  ...
  try {
    ...
    // Calculate total liquidity = sum of all bid sizes + all ask sizes
    const totalBidLiq = bids.reduce((sum, lvl) => sum + (lvl.shares || 0), 0);
    const totalAskLiq = asks.reduce((sum, lvl) => sum + (lvl.shares || 0), 0);
    const totalLiquidity = totalBidLiq + totalAskLiq;
    const val = Number.isFinite(best) ? { price: best, size: bestSize, totalLiquidity } : null;
    ...
    const jobs = pairs.map((pair) =>
      withConcurrency(async () => {
        ...
        try {
          ...
          // Opinion liquidity: YES and NO orderbooks are mirrored (same book, inverted sides)
          // so we use only ONE token's totalLiquidity to avoid double-counting
          const opinionTotalLiquidity = opYesData?.totalLiquidity ?? opNoData?.totalLiquidity ?? 0;
...
export async function streamArbitageOpportunities({
  ...
  try {
    ...
    for (let i = 0; i < pairs.length; i += BATCH_SIZE) {
      ...
      // Keep quick scan capped, but allow full scan to return all matched opportunities.
