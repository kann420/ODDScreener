...
import { normalizePredictFunEventKey } from "@/lib/utils/predictfunEventFilter";
...
export async function scanArbitageOpportunities({ minArbPct = 0.1, minSimilarity = 0.9, limit = 50, priceMode = "asks", scanMode = "quick", platformA = "polymarket", platformB = "opinion", predictFunEvent = null } = {}) {
  ...
  const allowNegativeArb = (predictFunEventKey === "march-madness" || predictFunEventKey === "live") && ((platformA === "polymarket" && platformB === "predictfun") || (platformA === "predictfun" && platformB === "polymarket"));
    ...
    const jobs = pairs.map((pair) =>
      withConcurrency(async () => {
        ...
        try {
          ...
          const arbPct = best.arb * 100;
          const pairAllowsNegativeArb = allowNegativeArb || Boolean(pair._marchMadnessPair);
