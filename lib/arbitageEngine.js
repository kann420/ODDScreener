 ...
 */
export async function scanArbitageOpportunities({ minArbPct = 0.1, minSimilarity = 0.9, limit = 50, priceMode = "bids", scanMode = "quick", platformA = "polymarket", platformB = "opinion" } = {}) {
  const debug = [];
  ...
  try {
    ...
    debug.push({ step: "discovering_pairs", minSimilarity, priceMode, scanMode, maxTotalMarkets, platformA, platformB });
    console.log("[AUTO-SCAN] Discovering pairs:", platformA, "+", platformB, "| minSimilarity:", minSimilarity, "| priceMode:", priceMode, "| scanMode:", scanMode);
    const discoverStartTs = Date.now();

    const discoveryResult = useMultiPlatform
      ? await discoverMultiPlatformPairs({ platformA, platformB, minSimilarity, maxTotalMarkets })
      ...
      withConcurrency(async () => {
        ...
        try {
          ...
          if (pA === "opinion") {
            ...
            sideAUrl = pair._polyEventA?.slug ? polyEventUrlFromSlug(pair._polyEventA.slug, pt) : polyMarketUrlFromSlug(pair._polyMarketA?.slug || pair.polyMarketSlug, pt);
          ...
          if (pB === "polymarket") {
            ...
            sideBUrl = pair.polyEventSlug ? polyEventUrlFromSlug(pair.polyEventSlug, pt) : polyMarketUrlFromSlug(pair.polyMarketSlug, pt);
        ...
        try {
          ...
          if (pA === "opinion") {
            ...
            sideAUrl = pair._polyEventA?.slug
              ? polyEventUrlFromSlug(pair._polyEventA.slug, pt)
          ...
          if (pB === "polymarket") {
            ...
            sideBUrl = pair.polyEventSlug
              ? polyEventUrlFromSlug(pair.polyEventSlug, pt)
