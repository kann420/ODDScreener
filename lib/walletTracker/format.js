...
// Avatar files are in public/leaderboard/
const opinionWhales = {
  whales: {
    "0x34d8619e909aa917cd176b733a5bfbafcdc0483e": { name: "beoakd", avatar: "beoakd.png" },
    "0x0edf1a0759ced1b9a09413ff774404c56e296d53": { name: "reikd", avatar: "reikd.png" },
    "0x9f5948c84d2567bd5ff9127ac14898334e66d002": { name: "Big_Winner", avatar: "Big_Winner.png" },
    "0x8a3d9b06ab28daddbae7f51c1f1d83bd9c1bd7a2": { name: "chealce", avatar: "chealce.png" },
    "0x578608c2be0004e501016a13ced75b35e33466b0": { name: "HNB", avatar: "HNB.png" },
    "0x06a881c591e1cad6651b97d0e06b6ab4b4ca52ba": { name: "jax", avatar: "jax.jpg" },
    "0x91a75b2554de4c70c393025a411a4064276303f4": { name: "mmmmmmm", avatar: "mmmmmmm.png" },
    "0xe44d4bb0a00b54ca6c7d8066aa04727842666666": { name: "zong", avatar: "zong.png" },
    "0x4bf5e3d097ff2ec2bd082166042cfedb49b00000": { name: "세계", avatar: "세계.png" },
    "0x98e8ffae7df736663a955e2b4b4e5f020042230d": { name: "lo", avatar: "lo.png" },
    "0x64a21f59af6f5592bd03a0b240592dd5aaec7517": { name: "Bishop", avatar: "Bishop.png" },
    "0x8bebef60dddfaaa451a5867f704ecb431ad8c77a": { name: "User_d8c77a", avatar: "User_d8c77a.png" },
    "0x8604ce9941d4f1ae20fe02e0d004ceca41393e0f": { name: "fight", avatar: "fight.png" },
    "0xacdee9bf6a824632b01e94fddbd98fac50d7d546": { name: "ak88", avatar: "ak88.png" },
    "0x82bba64fc3a7f87f3d3e639de7b044c644921064": { name: "User_921064", avatar: "User_921064.png" },
    "0x2d57911f5b0bc2e276bc6181cc64d9e94c5cb92b": { name: "NothingToSay", avatar: "NothingToSay.png", topPnl: true }
  }
};
...
/**
 * Normalize a wei-format value from Opinion API
 * 
 * Opinion API sometimes returns values in two formats:
 * - Normal USD (e.g., 1000 for $1000)
 * - Wei format (18 decimals, e.g., 1000000000000000000000 for $1000)
 * 
 * Detection: If absolute value > 1e10, it's in wei format and needs division by 1e18
 * 
 * @param {*} value - Raw value from API (can be positive or negative)
 * @returns {number} - Normalized USD value
 */
export function normalizeWeiValue(value) {
  const raw = num(value);
  
  // If absolute value is extremely large (> 10 billion), it's in wei format
  // Normal trades rarely exceed $10 billion, so this is a safe threshold
  if (Math.abs(raw) > 1e10) {
    return raw / 1e18;
  }
...
/**
 * Normalize USD amount from Opinion API
 * 
 * Opinion API returns usdAmount in two formats:
 * - Normal USD (e.g., 1000 for $1000)
 * - Wei format (18 decimals, e.g., 1000000000000000000000 for $1000)
 * 
 * Detection: If value > 1e10, it's in wei format and needs division by 1e18
 * 
 * @param {*} usdAmount - Raw usdAmount from API
 * @param {*} fallbackAmount - Fallback value (trade.amount) if usdAmount is empty
 * @returns {number} - Normalized USD value
 */
export function normalizeUsdAmount(usdAmount, fallbackAmount) {
  // Try usdAmount first, then fallback
  let raw = num(usdAmount);
  if (raw === 0) {
    raw = num(fallbackAmount);
  }
  
  return normalizeWeiValue(raw);
}
