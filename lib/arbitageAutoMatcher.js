...
function checkWhitelistMatch(opinionTitle, polyEvents) {
  ...
  for (const rule of MANUAL_MATCH_WHITELIST) {
    // Check if Opinion title matches the pattern
    const opMatches = rule.opinionPattern instanceof RegExp 
    ...
    for (const event of polyEvents) {
      ...
      const eventMatches = rule.polyPattern instanceof RegExp
      ...
      for (const market of markets) {
        ...
        const marketMatches = rule.polyPattern instanceof RegExp
...
export async function discoverArbitagePairs({ minSimilarity = 0.5, maxTotalMarkets = 100, onProgress = () => {} } = {}) {
  ...
  // Sort: whitelisted first, then by similarity descending
  pairs.sort((a, b) => {
    // Whitelisted pairs come first
