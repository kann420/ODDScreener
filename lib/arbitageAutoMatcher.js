...
} from "@/lib/utils/arbitragePredictFunMatching";
import dns from "dns";
import https from "https";

// Create custom DNS resolver using Cloudflare DNS (bypasses local DNS issues)
const cloudflareResolver = new dns.Resolver();
cloudflareResolver.setServers(["1.1.1.1"]);
console.log("[AutoMatcher][DNS] Using Cloudflare DNS resolver: 1.1.1.1");
...
function findPolyMatchForOpinionMarket(opinionMarket, polyEvents, minSimilarity = 0.5, { enableWhitelist = true } = {}) {
  ...
  for (const event of polyEvents) {
    ...
    if (combinedScore >= minSimilarity && combinedScore > bestScore) {
      ...
      if (hasFdvSeriesMismatch(opTitle, eventTitle)) {
        continue;
      }
    ...
    for (const market of markets) {
      ...
      if (marketCombinedScore >= minSimilarity && marketCombinedScore > bestScore) {
        ...
        if (hasFdvSeriesMismatch(opTitle, marketQuestion, "", market.groupItemTitle || market.title || "")) {
          continue;
        }
...
function matchCategoricalOutcomes(opinionCategorical, polyEventOrEvents) {
  ...
  const opinionParentTitle =
    opinionCategorical.marketTitle || opinionCategorical.tittle || opinionCategorical.title || "";
...
export async function discoverArbitagePairs({ minSimilarity = 0.5, maxTotalMarkets = 100, onProgress = () => {} } = {}) {
  ...
  const [opinionMarkets, polyEvents] = await Promise.all([
    ...
    fetchAllPolymarketEvents({ limit: 200, maxPages: isFullScan ? 30 : 15, includeWhitelistSearch: true }),
  ]);
...
function predictfunMarketsToPolyFormat(markets) {
  ...
  return [...groups.entries()].map(([groupKey, groupMarkets]) => {
    ...
    const eventTitle = buildPredictFunPseudoEventTitle(groupMarkets, groupKey);
    ...
    const pseudoMarkets = groupMarkets.map((m) => {
      ...
      const pseudoMarketText = buildPredictFunPseudoMarket(m, eventTitle);

      return {
        id: marketId,
        marketId: marketId,
        question: pseudoMarketText.question,
        title: pseudoMarketText.title,
        groupItemTitle: pseudoMarketText.groupItemTitle,
        slug: pseudoMarketText.pseudoSlug,
        outcomes: ["Yes", "No"],
        clobTokenIds: JSON.stringify([`pfyes:${marketId}`, `pfno:${marketId}`]),
        active: true,
        closed: false,
        image: m.imageUrl || "",
        startDate: m._categoryStartsAt || null,
        ...
        _categoryStartsAt: m._categoryStartsAt || null,
        _categoryEndsAt: m._categoryEndsAt || null,
        _categoryMarketVariant: m._categoryMarketVariant || null,
        _categoryStatus: m._categoryStatus || null,
        _categoryIsVisible: m._categoryIsVisible ?? null,
...
function getPolymarketPredictFunPairKey(pair) {
  const platformA = String(pair?._platformA || "");
  const platformB = String(pair?._platformB || "");
  const pairType = String(pair?.type || "");
  const sideAId = String(pair?.opinionMarketId || pair?.opinionTitle || pair?.id || "").trim();
  const sideBId = String(
    pair?.polyMarket?.marketId ||
      pair?.polyMarket?.id ||
      pair?.polyMarketSlug ||
      pair?._predictfunMarketB?.marketId ||
      pair?._predictfunMarketB?.id ||
      ""
  ).trim();

  return [platformA, platformB, pairType, sideAId, sideBId].join("|");
}
...
export async function discoverMultiPlatformPairs({
  ...
  let pairMatchMinSimilarity = minSimilarity;
  ...
  try {
    if (platformA === "opinion" && platformB === "probable") {
      ...
      const [polyEvents, probEvents] = await Promise.all([
        fetchAllPolymarketEvents({ limit: 200, maxPages: isFullScan ? 30 : 15, includeWhitelistSearch: false }),
        fetchAllProbableEvents(),
      ]);
      ...
      // SPECIAL PATH: conditionId-first, with live allowing fuzzy fallback
      ...
      const shouldFilterLive = normalizedPredictFunEvent === "live";
      const shouldFilterPredictFunCategory = shouldFilterMarchMadness || shouldFilterLive;
      let shouldEnableFuzzyFallback = false;
      let fuzzyPfMarkets = [];
      const pfMaxMarkets = isFullScan ? 600 : maxTotalMarkets > 80 ? 300 : 150;
      const mmSupplementMaxMarkets = isFullScan ? 1000 : 700;
      const fetchStart = Date.now();
      const skipPolyGammaFetch = shouldFilterLive;
      ...
      if (shouldFilterPredictFunCategory) {
        ...
        console.log(
          `[AutoMatcher][Poly-PredictFun] ${normalizedPredictFunEvent || "unfiltered"} filter retained ${pfMarkets.length}/${mergedPfMarkets.size} PF markets`
        );
      }

      fuzzyPfMarkets = pfMarkets.filter((m) => {
        const ids = Array.isArray(m?.polymarketConditionIds) ? m.polymarketConditionIds : [];
        return ids.length === 0;
      });
      shouldEnableFuzzyFallback = shouldFilterMarchMadness && fuzzyPfMarkets.length > 0;
      pairMatchMinSimilarity = shouldEnableFuzzyFallback ? Math.min(minSimilarity, 0.5) : minSimilarity;

      logPhase(
        "fetch_markets",
        fetchStart,
        `polyEvents=${polyEvents?.length || 0}, pfMarkets=${pfMarkets?.length || 0}, mmSupplement=${marchMadnessPfMarkets.length}, filter=${shouldFilterPredictFunCategory ? "on" : "off"}`
      );
      ...
      // Fuzzy fallback is only for March Madness-style markets.
      // Live should stay conditionId-only to keep matching fast and exact.
      if (shouldEnableFuzzyFallback && pfMarkets.length > 0) {
        const uniqueSlugs = [...new Set(pfMarkets.map((m) => m?.categorySlug).filter(Boolean))];
        if (uniqueSlugs.length > 0) {
          const categories = await mapWithConcurrency(
            uniqueSlugs,
          ...
          const slugToCategoryData = new Map(
            categories.map((item) => [
              item.slug,
              {
                startsAt: item?.category?.startsAt || null,
                endsAt: item?.category?.endsAt || null,
                marketVariant: item?.category?.marketVariant || null,
                status: item?.category?.status || null,
                isVisible: item?.category?.isVisible ?? null,
              },
            ])
          );
          for (const pfMarket of pfMarkets) {
            const catData = slugToCategoryData.get(pfMarket?.categorySlug) || {};
            pfMarket._categoryStartsAt = catData.startsAt || null;
            pfMarket._categoryEndsAt = catData.endsAt || null;
            pfMarket._categoryMarketVariant = catData.marketVariant || null;
            pfMarket._categoryStatus = catData.status || null;
            pfMarket._categoryIsVisible = catData.isVisible ?? null;
            if (pfMarket._predictfunRaw) {
              pfMarket._predictfunRaw._categoryStartsAt = catData.startsAt || null;
              pfMarket._predictfunRaw._categoryEndsAt = catData.endsAt || null;
              pfMarket._predictfunRaw._categoryMarketVariant = catData.marketVariant || null;
              pfMarket._predictfunRaw._categoryStatus = catData.status || null;
              pfMarket._predictfunRaw._categoryIsVisible = catData.isVisible ?? null;
            }
          }
        }
      }
      ...
      if (shouldFilterLive) {
        ...
        for (const pfMkt of pfMarkets) {
          ...
          for (const cid of pfCondIds) {
            const cidLower = String(cid || "").trim().toLowerCase();
            if (cidLower) liveConditionIds.push(cidLower);
          }
        }

        const uniqueLiveConditionIds = [...new Set(liveConditionIds)];
        const liveMatches = await mapWithConcurrency(
          uniqueLiveConditionIds,
          async (conditionId) => {
        ...
        let liveAdded = 0;
        for (const fallback of liveMatches) {
        ...
        console.log(
          `[AutoMatcher][Poly-PredictFun][Live] Direct CLOB lookup added ${liveAdded}/${uniqueLiveConditionIds.length} Polymarket markets`
        );
      } else {
        for (const event of polyEvents) {
          if (!isPolyEventOpen(event)) continue;
        ...
        const fallbackSourceMarkets = pfMarkets;
        if (fallbackSourceMarkets.length > 0) {
          const missingConditionIds = [];
          for (const pfMkt of fallbackSourceMarkets) {
            const pfCondIds = Array.isArray(pfMkt?.polymarketConditionIds) ? pfMkt.polymarketConditionIds : [];
            for (const cid of pfCondIds) {
          ...
          // Bounded CLOB fallback: useful when Gamma paging misses a conditionId,
          // but never fetch unbounded (rate limits + latency).
          const maxClobFallback = isFullScan ? 120 : 60;
          const uniqueMissingConditionIds = [...new Set(missingConditionIds)].slice(0, maxClobFallback);
          if (uniqueMissingConditionIds.length > 0) {
            ...
            console.log(
              `[AutoMatcher][Poly-PredictFun] ${normalizedPredictFunEvent || "all"} CLOB fallback added ${fallbackAdded}/${uniqueMissingConditionIds.length} missing Polymarket markets`
            );
          }
        }
      }
      ...
      logPhase("build_condition_index", indexStart, `indexedConditionIds=${polyByConditionId.size}${shouldFilterLive ? ", source=clob-live" : ""}`);
      ...
      if (matchedCategorySlugs.length) {
        ...
        const slugToCategoryData = {};
        for (const c of categories) {
          slugToCategoryData[c.slug] = {
            endsAt: c?.category?.endsAt || null,
            startsAt: c?.category?.startsAt || null,
            marketVariant: c?.category?.marketVariant || null,
            status: c?.category?.status || null,
            isVisible: c?.category?.isVisible ?? null,
          };
        }
        for (const pair of condPairs) {
          const pfMarket = pair?._predictfunMarketB;
          if (!pfMarket) continue;
          const catData = slugToCategoryData[pfMarket._categorySlug] || {};
          pfMarket.endDate = catData.endsAt || null;
          pfMarket.startDate = catData.startsAt || null;
          pfMarket._categoryMarketVariant = catData.marketVariant || null;
          pfMarket._categoryStatus = catData.status || null;
          pfMarket._categoryIsVisible = catData.isVisible ?? null;
          if (pfMarket._predictfunRaw) {
      ...
      if (shouldEnableFuzzyFallback) {
        sideAMarkets = polyEventsToFlatMarkets(polyEvents);
        sideBEvents = predictfunMarketsToPolyFormat(fuzzyPfMarkets);
        allowEmptySideB = false;
      } else {
        sideAMarkets = [];
        sideBEvents = [];
      ...
      logPhase(
        "total_branch",
        branchStart,
        `exactOnly=${shouldFilterLive ? "live" : "mixed"}, fuzzyFallback=${shouldEnableFuzzyFallback ? "on" : "off"}, conditionIdPairs=${conditionIdPairs.length}`,
      );
    } else if (
    ...
    const pushUniquePair = (pair) => {
      const key = getPolymarketPredictFunPairKey(pair);
    ...
    if (conditionIdPairs.length > 0) {
      for (const pair of conditionIdPairs) {
        pushUniquePair(pair);
      }
      ...
      for (let i = 0; i < sideAMarkets.length; i++) {
        ...
        const match = findPolyMatchForOpinionMarket(market, sideBEvents, pairMatchMinSimilarity, { enableWhitelist: false });
      ...
      if (!matchedMarket && matchedEvent?.markets?.length > 0) {
        if (matchedEvent.markets.length === 1) {
          ...
          for (const m of matchedEvent.markets) {
            if (!isPolyMarketTradable(m)) continue;
            const mq = m.question || m.title || "";
            if (hasFdvSeriesMismatch(opTitle, mq, "", m.groupItemTitle || m.title || "")) {
              continue;
            }
            ...
            if (bestM && bestS >= pairMatchMinSimilarity) matchedMarket = bestM;
        }
      }
        ...
        if (market.isCategorical) {
          ...
          if (!sideBMarket && matchedEvent?.markets?.length > 0) {
            ...
            for (const m of markets) {
              ...
              const sim = calculateSimilarity(opTitle, mq);
              if (sim >= pairMatchMinSimilarity && sim > bestS) { bestS = sim; bestM = m; }
            }
