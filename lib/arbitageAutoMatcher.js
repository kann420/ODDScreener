...
export async function discoverMultiPlatformPairs({
  ...
  try {
    if (platformA === "opinion" && platformB === "probable") {
      ...
      const [polyEvents, pfMarkets] = await Promise.all([
      ...
      logPhase("fetch_markets", fetchStart, `polyEvents=${polyEvents?.length || 0}, pfMarkets=${pfMarkets?.length || 0}, whitelistSearch=off`);

      if (!pfMarkets || pfMarkets.length === 0) {
        return {
          pairs: [],
      ...
      if (matchedCategorySlugs.length) {
        ...
        const slugToEndsAt = {};
        for (const c of categories) slugToEndsAt[c.slug] = c?.category?.endsAt || null;
        for (const pair of condPairs) {
          const pfMarket = pair?._predictfunMarketB;
          if (!pfMarket) continue;
          const endsAt = slugToEndsAt[pfMarket._categorySlug] || null;
          pfMarket.endDate = endsAt;
          if (pfMarket._predictfunRaw) pfMarket._predictfunRaw._categoryEndsAt = endsAt;
        }
      }
