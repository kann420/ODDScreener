...
export async function getPredictFunPriceHistory(marketId, { limit = 50 } = {}) {
  ...
  try {
    ...
    const points = allMatches
      .map((m) => {
        const rawPrice = Number(m.priceExecuted);
        const price = rawPrice > 1 ? rawPrice / 1e18 : rawPrice;
        const t = m.timestamp
          ? new Date(m.timestamp).getTime()
          : m.executedAt
            ? new Date(m.executedAt).getTime()
            : 0;
    ...
    MATCHES_CACHE.set(key, { data: points, fetchedAt: Date.now() });
