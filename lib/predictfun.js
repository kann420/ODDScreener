 ...
 * @param {string|number|Array<string|number>} options.tagIds - Optional tag filter forwarded to /v1/markets
 * @param {string} options.sort - Optional sort key forwarded to /v1/markets (default VOLUME_24H_DESC)
 */
export async function fetchAllPredictFunMarkets({ pageSize = 100, maxMarkets = 600, enrichCategoryEndDate = true, tagIds = null, sort = "VOLUME_24H_DESC" } = {}) {
  const allMarkets = [];
  let cursor = null;
  let pages = 0;
  ...
  try {
    do {
      const params = {
        ...
        sort: String(sort || "VOLUME_24H_DESC"),      // highest volume first (best for arb)
        includeStats: "true",         // inline stats (volume24hUsd, totalLiquidityUsd)
      };
      if (tagIds != null) {
        params.tagIds = Array.isArray(tagIds) ? tagIds.join(",") : String(tagIds);
      }
 ...
 */
export async function getPredictFunOrderbookData(marketId, decimalPrecision = null, { forceFresh = false } = {}) {
 ...
 */
export async function getPredictFunSideOrderbook(prefixedTokenId, { forceFresh = false } = {}) {
  const isNo = prefixedTokenId.startsWith("pfno:");
  const marketId = prefixedTokenId.replace(/^pf(?:yes|no):/, "");
  if (!marketId) return null;

  const data = await getPredictFunOrderbookData(marketId, null, { forceFresh });
  if (!data) return null;
 ...
 */
export async function getPredictFunDisplayOrderbook(prefixedTokenId, { forceFresh = false } = {}) {
  const isNo = prefixedTokenId.startsWith("pfno:");
  const marketId = prefixedTokenId.replace(/^pf(?:yes|no):/, "");
  if (!marketId) return null;
