...
export async function getPredictFunMarket(marketId) {
  ...
  if (cached?.decimalPrecision != null) {
    return {
      id: Number(marketId),
      decimalPrecision: cached.decimalPrecision,
    };
  }
  ...
  try {
    ...
    const data = res?.success ? res.data : null;
    if (data) rememberPredictFunMarketMeta(data);
    return data;
  } catch (err) {
 ...
 */
export async function fetchAllPredictFunMarkets({ pageSize = 50, maxMarkets = 600, enrichCategoryEndDate = true } = {}) {
  const allMarkets = [];
  let cursor = null;
  let pages = 0;
    ...
    if (enrichCategoryEndDate) {
      ...
      for (let i = 0; i < uniqueSlugs.length; i += BATCH) {
        ...
        for (let j = 0; j < batch.length; j++) {
          ...
          slugToEndsAt[batch[j]] = cat?.endsAt || null;
        }
      }
      for (const m of allMarkets) {
        m._categoryEndsAt = slugToEndsAt[m.categorySlug] || null;
      }
...
function obCacheSet(key, val) {
  if (OB_CACHE.size > 500) {
    const oldest = OB_CACHE.keys().next().value;
    OB_CACHE.delete(oldest);
  }
