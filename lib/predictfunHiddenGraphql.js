...
export async function fetchPredictFunAccountInfo(walletAddress) {
  ...
  try {
    ...
    return {
      ...
      leaderboard: acc.leaderboard || null,
      weeklyPointsHistory: Array.isArray(acc.weeklyPointsHistory) ? acc.weeklyPointsHistory : [],
      points: Number(acc.leaderboard?.totalPoints ?? 0),
    };
  } catch (err) {
  ...
  for (let page = 0; page < 20; page += 1) {
    ...
    if (!account) {
      account = {
        ...
        leaderboard: accountNode.leaderboard || null,
        weeklyPointsHistory: Array.isArray(accountNode.weeklyPointsHistory) ? accountNode.weeklyPointsHistory : [],
        points: Number(accountNode.leaderboard?.totalPoints ?? 0),
      };
    }
