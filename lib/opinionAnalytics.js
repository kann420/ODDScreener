...
export async function analyticsFetch(path, options = {}) {
  ...
  try {
    ...
    // Only log if not a connection refused error (reduces noise when server is down)
    if (error.code !== 'ECONNREFUSED' && error.name !== 'AbortError') {
      console.error("Analytics API error:", error.message);
    } else {
      console.warn(`[Analytics] API unavailable: ${error.message || error.name}`);
    }
    return { success: false, error: { message: error.message || 'API unavailable' } };
  }
}
