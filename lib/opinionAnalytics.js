 ...
 * NOTE: Analytics API (opinion.api.predictscan.dev:10001) may be unavailable.
 * When unavailable, returns { success: false } and the app should gracefully fallback.
 */
export async function analyticsFetch(path, options = {}) {
  ...
  try {
    ...
    // Suppress common connection errors when Analytics server is down
    const msg = error.message || error.name || '';
    const isConnectionError = error.code === 'ECONNREFUSED' || error.name === 'AbortError' || msg.includes('fetch failed');
    if (!isConnectionError) {
      console.error("Analytics API error:", msg);
    }
    // Silent fallback - page.jsx handles fallback to Opinion API
    return { success: false, error: { message: msg || 'API unavailable' } };
  }
}
 ...
 * We validate by sampling one child from each parent via Opinion Trade API
 * to filter out resolved parents.
 */
export async function getMultiOutcomeMarkets() {
  ...
  // and only activated ones (initial filter)
  const multiOutcomeRaw = marketsResponse.data
    .filter(m => m.parentEventId && m.statusEnum === "Activated");

  // =====================================================
  // Validate parents by sampling one child per parent
  // =====================================================
  
  // Group by parentEventId
  const parentGroups = new Map(); // parentEventId -> [markets]
  for (const m of multiOutcomeRaw) {
    const pid = String(m.parentEventId);
  ...
  // Get sample market IDs (one per parent)
  const sampleMarkets = [];
  for (const [pid, markets] of parentGroups) {
    sampleMarkets.push({
      parentEventId: pid,
  ...
  // Batch validate samples via Opinion Trade API (max 50 concurrent)
  const resolvedParents = new Set();
  const BATCH_SIZE = 50;
  
  for (let i = 0; i < sampleMarkets.length; i += BATCH_SIZE) {
    const batch = sampleMarkets.slice(i, i + BATCH_SIZE);
    const results = await Promise.allSettled(
      batch.map(async ({ parentEventId, sampleMarketId }) => {
        try {
          const r = await opinionFetch(`/market/${sampleMarketId}`);
          ...
          if (data) {
            ...
            const isResolved = 
              ...
              String(data.statusEnum || '').toLowerCase().includes('resolved') ||
              String(data.statusEnum || '').toLowerCase().includes('resolving') ||
              (data.resolvedAt && Number(data.resolvedAt) > 0) ||
              (data.resultTokenId && data.resultTokenId !== '');
            
            if (isResolved) {
              return { parentEventId, resolved: true };
            }
          }
          return { parentEventId, resolved: false };
        } catch {
          return { parentEventId, resolved: false };
        }
      })
    ...
    for (const result of results) {
      if (result.status === 'fulfilled' && result.value.resolved) {
        resolvedParents.add(result.value.parentEventId);
      }
    }
  }
  
  console.log(`[MultiOutcome] Validated ${sampleMarkets.length} parents, ${resolvedParents.size} resolved`);
  
  // Filter out markets whose parent is resolved
  const validatedMarkets = multiOutcomeRaw.filter(m => 
    !resolvedParents.has(String(m.parentEventId))
  );
  
  console.log(`[MultiOutcome] Filtered: ${multiOutcomeRaw.length} -> ${validatedMarkets.length} markets`);

  // Get all market IDs to fetch volume for
  const marketIds = validatedMarkets.map(m => m.marketId);
  ...
  const multiOutcomeMarkets = validatedMarkets.map(m => {
