...
async function _fetchCategoricalChildrenInternal(options = {}) {
  ...
  // Calculate how many pages to fetch (capped by maxParents)
  const maxParentPages = Math.min(Math.ceil(maxParents / PAGE_SIZE), Math.ceil(totalAvailable / PAGE_SIZE), 20);
