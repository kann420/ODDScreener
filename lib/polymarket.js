...
async function fetchJsonDirectIp(hostname, path, opts = {}) {
  const { timeoutMs = 15000, method = "GET" } = opts;
...
export async function getPolyClobMarkets(conditionIds) {
  ...
  // Fetch markets in parallel (with some batching to avoid overwhelming)
  await Promise.all(
    ids.map(async (cid) => {
      const market = await getPolyClobMarket(cid);
      if (market) {
        out.set(cid, market);
      }
    })
...
export async function getPolyBestBids(tokenIds) {
  ...
  // Fetch prices in parallel using /price endpoint
  await Promise.all(
    ids.map(async (tid) => {
      const price = await getPolyPriceForToken(tid);
...
export async function getPolyOrderbooks(tokenIds) {
  ...
  await Promise.all(
    ids.map(async (tid) => {
      const ob = await getPolyOrderbook(tid);
      if (ob) {
        out.set(tid, ob);
      }
    })
...
export async function getPolyBestAsks(tokenIds) {
  ...
  await Promise.all(
    ids.map(async (tid) => {
      const price = await getPolyAskForToken(tid);
      if (Number.isFinite(price)) {
        out.set(tid, price);
      }
    })
