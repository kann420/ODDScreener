...
// Cloudflare DNS resolver to bypass local DNS issues
const cloudflareResolver = new dns.Resolver();
cloudflareResolver.setServers(["1.1.1.1"]);
console.log("[Poly][DNS] Using Cloudflare DNS resolver: 1.1.1.1");
...
export async function getPolyClobMarket(conditionId) {
  ...
  try {
    ...
    console.error("[Poly] getPolyClobMarket failed:", err?.message || err);
    return null;
  }
}
...
export async function getPolyClobMarkets(conditionIds) {
  ...
  // Process in pages of up to 500 ids to keep large scans bounded.
  const PAGE_SIZE = 500;
  for (let i = 0; i < ids.length; i += PAGE_SIZE) {
    const batch = ids.slice(i, i + PAGE_SIZE);

    // Rate-limited concurrent fetch (max 6 parallel, 8 req/s)
    await mapWithConcurrency(batch, async (cid) => {
...
function parseMaybeJsonArray(v) {
  ...
  try {
    ...
    // Not valid JSON array, try manual parsing below
  }
 ...
 * Cached for 15 seconds to avoid redundant API calls during scans
 * @param {string} tokenId - Token ID
 * @returns {Object} - { bids: [{price, size}], asks: [{price, size}], bestBid, bestAsk, bestBidSize, bestAskSize }
 */
const _polyObCache = new Map();
const _POLY_OB_TTL = 15000; // 15 seconds
const _MAX_POLY_OB_CACHE = 2000;

// One-time sort order verification flag
let _sortOrderVerified = false;

export async function getPolyOrderbook(tokenId, { forceFresh = false } = {}) {
  if (!tokenId) return null;

  // Check cache first
  const cacheKey = `pOB:${tokenId}`;
  if (!forceFresh) {
    const cached = _polyObCache.get(cacheKey);
  ...
  try {
    ...
    // Verify sort order on first successful fetch (log once)
    if (!_sortOrderVerified && bids.length >= 2) {
      const firstBid = Number(bids[0]?.price);
      const lastBid = Number(bids[bids.length - 1]?.price);
      if (firstBid > lastBid) {
        console.warn("[Poly OB] WARNING: Bids are DESC (first > last). Best bid = bids[0], NOT bids[last]. Fix getPolyOrderbook!");
      } else {
        console.log("[Poly OB] Verified: Bids are ASC (first < last). Best bid = bids[last]. OK.");
      }
      _sortOrderVerified = true;
    }

    // CLOB sorts bids ASC (best bid = last), asks DESC (best ask = last)
    // If API changes sort order, the verification log above will warn you
    ...
    // Compute total liquidity (sum of all bid+ask sizes) for parity with Opinion/Probable OBs
    const totalBidLiq = bids.reduce((sum, b) => sum + Number(b.size || 0), 0);
    const totalAskLiq = asks.reduce((sum, a) => sum + Number(a.size || 0), 0);
    ...
    if (!forceFresh) {
      if (_polyObCache.size >= _MAX_POLY_OB_CACHE) {
        const oldest = _polyObCache.keys().next().value;
        _polyObCache.delete(oldest);
      }
      _polyObCache.set(cacheKey, { val: result, exp: Date.now() + _POLY_OB_TTL });
    }
    return result;
  } catch (err) {
    console.error("[Poly OB] getPolyOrderbook failed:", err?.message || err);
    if (!forceFresh) {
      _polyObCache.set(cacheKey, { val: null, exp: Date.now() + 5000 });
    }
    return null;
  }
}
...
async function getPolyPriceForToken(tokenId) {
  ...
  try {
    ...
    console.error("[Poly] getPolyPriceForToken failed:", err?.message || err);
    return null;
  }
}
...
async function getPolyAskForToken(tokenId) {
  ...
  try {
    ...
    console.error("[Poly] getPolyAskForToken failed:", err?.message || err);
    return null;
  }
}
...
export async function getPolyBestBids(tokenIds) {
  ...
  // Rate-limited concurrent fetch (max 30 parallel, 100 req/s)
 ...
 */
export async function getPolyOrderbooks(tokenIds, { forceFresh = false } = {}) {
  const ids = (tokenIds || []).map(String).filter(Boolean);
  const out = new Map();
  if (!ids.length) return out;
  ...
  // Rate-limited concurrent fetch (max 30 parallel, 100 req/s)
  await mapWithConcurrency(ids, async (tid) => {
    const ob = await polyRateLimiter.run(() => getPolyOrderbook(tid, { forceFresh }));
...
export function polyMarketUrlFromSlug(slug, title = null) {
  if (!slug && !title) return "https://polymarket.com?r=User42069";
  ...
  return `https://polymarket.com/market/${finalSlug}?r=User42069`;
}
...
export function polyEventUrlFromSlug(slug, title = null) {
  ...
  const finalSlug = slug || titleToSlug(title);
  // Don't encode the slug - Polymarket uses clean URLs like /event/fed-decision-january
  return `https://polymarket.com/event/${finalSlug}?r=User42069`;
}

// New Polymarket URL format: /event/{eventSlug}/{marketSlug}
export function polyEventMarketUrl(eventSlug, marketSlug) {
  if (eventSlug && marketSlug) {
    return `https://polymarket.com/event/${eventSlug}/${marketSlug}?r=User42069`;
  }
  if (eventSlug) return polyEventUrlFromSlug(eventSlug);
  if (marketSlug) return polyMarketUrlFromSlug(marketSlug);
  return "https://polymarket.com?r=User42069";
}
