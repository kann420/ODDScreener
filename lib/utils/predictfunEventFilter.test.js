...
test("isPredictFunLiveMarket accepts live-variant categories that are currently inside the category window", () => {
  const now = Date.now();
  const category = {
    status: "OPEN",
    isVisible: true,
  ...
  assert.equal(isPredictFunLiveMarket(market, category), true);
  assert.equal(matchesPredictFunEventFilter(market, "live", category), true);
});

test("isPredictFunLiveMarket rejects a future Predict.fun category even if the market is open", () => {
  const now = Date.now();
  const category = {
    status: "OPEN",
    isVisible: true,
  ...
  assert.equal(isPredictFunLiveMarket(market, category), false);
  assert.equal(matchesPredictFunEventFilter(market, "live", category), false);
});

test("isPredictFunLiveMarket rejects a default-variant category even inside the time window", () => {
  const now = Date.now();
  const category = {
    status: "OPEN",
    isVisible: true,
...
test("isPredictFunLiveMarket can fall back to category fields copied onto a raw market", () => {
  const now = Date.now();
  const market = {
    isVisible: true,
    tradingStatus: "OPEN",
    resolution: null,
    _categoryMarketVariant: "CRYPTO_UP_DOWN",
    _categoryStatus: "OPEN",
    _categoryIsVisible: true,
    _categoryStartsAt: new Date(now - 60_000).toISOString(),
    _categoryEndsAt: new Date(now + 60_000).toISOString(),
  };

  assert.equal(isPredictFunLiveMarket(market), true);
  assert.equal(matchesPredictFunEventFilter(market, "live"), true);
});
