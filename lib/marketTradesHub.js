...
if (!hub) {
  hub = {
    ...
    clients: new Map(),
    // Map: key -> Array of recent trades (in-memory cache)
    trades: new Map(),

    // ✅ NEW: Pin counts so hub can keep tracking even without any browser clients
    // Map: key -> number
    pins: new Map(),
...
function ensureWS() {
  ...
  hub.ws.on("open", () => {
    ...
    // ✅ Re-subscribe to all keys that have active clients OR pins
    const keys = new Set([...hub.clients.keys(), ...hub.pins.keys()]);
    for (const key of keys) subscribeKey(key);
  });
  ...
  hub.ws.on("message", (data) => {
    try {
      ...
      // Store & broadcast by BOTH keys when available
...
export function addMarketClient(sub, client) {
  ...
  return () => {
    ...
    if (set.size === 0) {
      const pinned = (hub.pins.get(key) || 0) > 0;

      // ✅ only unsubscribe & evict cache if NOT pinned
      if (!pinned) {
        hub.clients.delete(key);
        unsubscribeKey(key);

        setTimeout(() => {
          if (!hub.clients.has(key) && (hub.pins.get(key) || 0) === 0) {
            hub.trades.delete(key);
          }
        }, 60000);
      }
    }
...
/**
 * ✅ NEW: Pin a market/root so hub keeps tracking even without clients.
 * Example: pinMarket({ rootMarketId: 61 }) for multi-outcome parent markets.
 */
export function pinMarket(sub) {
  const key = makeKey(sub || {});
  if (!key) return;

  ensureWS();

  hub.pins.set(key, (hub.pins.get(key) || 0) + 1);

  // Ensure key exists so it will resubscribe on reconnect
  if (!hub.clients.has(key)) hub.clients.set(key, new Set());

  if (hub.wsReady) subscribeKey(key);
}

/**
 * ✅ NEW: Unpin. Will unsubscribe only if no clients + no pins left.
 */
export function unpinMarket(sub) {
  const key = makeKey(sub || {});
  if (!key) return;

  const cur = hub.pins.get(key) || 0;
  if (cur <= 1) hub.pins.delete(key);
  else hub.pins.set(key, cur - 1);

  const set = hub.clients.get(key);
  const hasClients = !!set && set.size > 0;
  const pinned = (hub.pins.get(key) || 0) > 0;

  if (!hasClients && !pinned) {
    hub.clients.delete(key);
    unsubscribeKey(key);
