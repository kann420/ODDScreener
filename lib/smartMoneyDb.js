...
function getDb() {
  ...
  // ✅ Add rootMarketId column if not exists (migration for existing DBs)
  try {
    db.exec(`ALTER TABLE trades ADD COLUMN rootMarketId INTEGER`);
  } catch {
  ...
  insertStmt = db.prepare(`
    ...
    VALUES (@ts, @marketId, @rootMarketId, @side, @amount, @price, @outcome, @marketTitle)
  `);
...
export function queryTrades({ hours = 24, minAmount = 200, limit = 200 } = {}) {
  ...
  const stmt = database.prepare(`
    SELECT ts, marketId, rootMarketId, side, amount, price, outcome, marketTitle
    FROM trades
    WHERE ts >= ? AND amount >= ?
    ORDER BY ts DESC
    LIMIT ?
